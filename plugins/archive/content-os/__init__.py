"""Content OS Plugin v2.5.0 — Full Content OS Implementation.

Complete 14-state lifecycle, 107 slop patterns (3 tiers + bonus),
4-route Idea Gate, Writer/Verifier dual-agent pipeline,
LLM-based postmortem with exact-line analysis, Buffer API publishing,
signal processing.
...
from .content_os_core import register_manager_action, STATE_LIFECYCLE
...
def register(ctx: Any) -> None:
    ...
    # ── Auto-enable GBrain MCP if available ──
    try:
        from model_tools import handle_function_call
        probe = handle_function_call(
            "mcp_gbrain_query",
            {"query": "probe", "limit": 1},
            skip_pre_tool_call_hook=True,
        )
        probe_result = json.loads(probe) if isinstance(probe, str) else probe
        if isinstance(probe_result, dict) and "error" not in probe_result:
            core.enable_gbrain()
            logger.info("GBrain MCP auto-enabled for Content OS")
    except Exception:
        logger.debug("GBrain MCP not available — running without semantic retrieval")
        pass

    # ══════════════════════════════════════════════════════════════
    # TOOL: content_os_manager (26 actions — full lifecycle + Buffer)
    # ══════════════════════════════════════════════════════════════
    ...
    manager_schema = {
        ...
        "description": (
            ...
            "slop scan (107 patterns), rubric scoring, signal processing, "
            "postmortem (LLM-based exact-line analysis), voice evolution, "
            "cross-run learning, pattern analysis, search, archive, "
            "Buffer API publishing (buffer_setup, buffer_send, buffer_status), "
            "Full pipeline mode (run_pipeline — single call for brief→draft→verify→score)."
        ),
        "parameters": {
            "type": "object",
            "properties": {
                ...
                "api_key": {"type": "string", "description": "Buffer API key (for buffer_setup action)"},
                "force": {"type": "boolean", "description": "Force state transition (bypass lifecycle validation)", "default": False},
            },
            "required": ["action"],
        },
    }
    ...
    ctx.register_cli_command(
        name="content",
        help="Content OS v2.5.0 — Full content production management",
        ...
        description=(
            ...
            "14-state lifecycle, 107 slop patterns (3 tiers + bonus), 4-route Idea Gate."
        ),
    )
    ...
    def post_tool_call(tool_name: str, args: Dict[str, Any], result: str, **kwargs):
        ...
        IMPORTANT: Only auto-updates if the new state is AHEAD in the
        lifecycle (no state regression). This prevents post_tool_call
        from overwriting manual state transitions like approved→scheduler_ready.
        """
        from pathlib import Path
        ...
        if target_path and ("runs/active/" in target_path or "runs\\active\\" in target_path):
            ...
            try:
                ...
                if filename in state_map:
                    ...
                    try:
                        current = core.get_state(slug)
                        new_idx = STATE_LIFECYCLE.index(new_state) if new_state in STATE_LIFECYCLE else -1
                        current_idx = STATE_LIFECYCLE.index(current) if current in STATE_LIFECYCLE else -1
                        # Only auto-update if new_state is ahead or equal in lifecycle
                        if new_idx >= current_idx:
                            ...
                            logger.debug(
                                "Skipped auto-update %s → %s (would regress from %s)",
                                slug, new_state, current
                            )
                    except Exception:
                        pass
            except Exception:
    ...
    # ══════════════════════════════════════════════════════════════
    logger.warning(
        "⚠️ Content-OS plugin v%s is DEPRECATED. "
        "Use 'prodinamik' tool (Prodinamik Engine) instead. "
        "Run: prodinamik action=... profile=content ... "
        "Content-OS will be archived in a future update.",
        VERSION,
    )

    # ══════════════════════════════════════════════════════════════
    # REGISTER BUFFER ACTIONS IN RUNTIME REGISTRY
    # ══════════════════════════════════════════════════════════════
    # These are registered in the _ACTION_HANDLERS dict so they
    # work even if tool_content_os_manager was imported before
    # buffer actions were available (hot-add without module reload).
    #
    # The handlers in the registry are thin wrappers that call the
    # corresponding ContentOSCore methods.

    async def _handle_buffer_setup(core_obj, buffer_args, **bw):
        return core_obj.buffer_setup(buffer_args.get("api_key"))

    async def _handle_buffer_send(core_obj, buffer_args, **bw):
        return core_obj.buffer_send(buffer_args.get("slug", ""))

    async def _handle_buffer_status(core_obj, buffer_args, **bw):
        return core_obj.buffer_status()

    register_manager_action("buffer_setup", _handle_buffer_setup)
    register_manager_action("buffer_send", _handle_buffer_send)
    register_manager_action("buffer_status", _handle_buffer_status)

    # Also register reload_actions for emergency hot-fix
    async def _handle_reload_actions(core_obj, reload_args, **bw):
        try:
            import importlib
            import sys
            ...
            from .content_os_core import register_manager_action as _ra
            return "🔄 Actions reloaded. New actions available in next session."
        except Exception as e:
            return f"❌ Reload failed: {e}"

    register_manager_action("reload_actions", _handle_reload_actions)
