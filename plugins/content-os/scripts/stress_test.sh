#!/usr/bin/env bash
# Content OS v2.4.0 — Cross-Platform Stress Test Suite
...
bold "╔══════════════════════════════════════════════════════════════╗"
bold "║     Content OS v2.4.0 — Stress Test Suite                 ║"
bold "║     $(date)                         ║"
bold "╚══════════════════════════════════════════════════════════════╝"
echo ""
...
from content_os_core import STATE_LIFECYCLE
print(len(STATE_LIFECYCLE))
")" "14-state lifecycle (expect 14)"

assert_contains "4" "$(python3 -c "
import sys; sys.path.insert(0, '$PLUGIN_DIR')
from content_os_core import IDEA_ROUTES
...
tmp = Path('/tmp/content-os-test-3')
core = ContentOSCore(tmp)
core.setup()
...
# Check slug exists
obj = tmp / 'runs' / 'active' / r1['slug'] / 'content-object.md'
assert obj.exists(), f'content-object.md missing for {r1["slug"]}'
...
from content_os_core import ContentOSCore
from pathlib import Path
tmp = Path('/tmp/content-os-test-4')
core = ContentOSCore(tmp); core.setup()
...
from content_os_core import ContentOSCore
from pathlib import Path
tmp = Path('/tmp/content-os-test-5')
core = ContentOSCore(tmp); core.setup()
...
from content_os_core import ContentOSCore
from pathlib import Path
tmp = Path('/tmp/content-os-test-6')
core = ContentOSCore(tmp); core.setup()
...
from content_os_core import ContentOSCore
from pathlib import Path
tmp = Path('/tmp/content-os-test-7')
core = ContentOSCore(tmp); core.setup()
...
import sys, tempfile
sys.path.insert(0, '$PLUGIN_DIR')
from content_os_core import ContentOSCore, STATE_LIFECYCLE, STATE_TRANSITIONS
from pathlib import Path

tmp = Path('/tmp/content-os-test-8')
core = ContentOSCore(tmp); core.setup()
...
tmp = Path('/tmp/content-os-test-9')
core = ContentOSCore(tmp); core.setup()
...
tmp = Path('/tmp/content-os-test-10')
core = ContentOSCore(tmp); core.setup()
...
tmp = Path('/tmp/content-os-test-11')
core = ContentOSCore(tmp); core.setup()
...
# Retriever
from content_os_core import tool_content_os_retriever
import json
result = json.loads(tool_content_os_retriever(core, {'category': 'run', 'slug': slug}))
assert isinstance(result, dict), f'Retriever should return dict: {type(result)}'
assert 'content-object' in result or 'idea' in result, f'Should retrieve run data: {list(result.keys())}'
...
tmp = Path('/tmp/content-os-test-12')
core = ContentOSCore(tmp); core.setup()
report = core.audit()
...
tmp2 = Path('/tmp/content-os-test-13')
core2 = ContentOSCore(tmp2)
