# Content-OS — Hermes Agent Plugin

## Proje Tanımı

**Content-OS**, Hermes Agent için geliştirilmiş bir **AI-Augmented Content Production System** plugin'idir.
...
Content-os, Hermes'in **General Plugin** sistemi üzerine inşa edilmiştir:

```
~/.hermes/plugins/content-os/
├── __init__.py              # register(ctx) - plugin kayıt
├── cli.py                   # CLI komutları (hermes content ...)
...
Content-os şu Hermes API'lerini kullanır:

| API | Kullanım |
|-----|----------|
| `ctx.register_tool()` | `content_os_manager`, `content_os_retriever` |
| `ctx.register_hook()` | `on_session_start`, `post_tool_call` |
| `ctx.register_command()` | `/content` slash command |
| `ctx.register_cli_command()` | `hermes content ...` CLI |
...
/usr/local/lib/hermes-agent/plugins/content-os/

# Project plugin
.hermes/plugins/content-os/  (HERMES_ENABLE_PROJECT_PLUGINS=true gerekli)
```
...
**Düzeltme:** `content_os_core.py`'deki tüm `write_text()` ve `read_text()` çağrılarına `encoding="utf-8"` eklendi.
...
```bash
# Yeni run oluştur
hermes content new "fikir" --slug optional-slug
...
3. **Bug bulma:** `content_os_core.py`'deki mantığı kontrol et
...
- ✅ `hermes content new` - Yeni run oluşturma
- ✅ `hermes content runs` - Tüm run'ları listeleme (8 run)
